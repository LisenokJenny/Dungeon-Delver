...
public class Dray : MonoBehaviour, IFacingMover, IKeyMaster {
	...
	void Update() {
		...
		// ----Обработка ввода с клавиатуры и управление режимами eMose----
		dirHeld = -1;
		for (int i = 0; i < 4; i++) {
	...
	void LateUpdate() {
		...
		// персонаж находится на плитка с дверью?
		int doorNum;
	...
	void OnCollisionEnter( Collision coll ) {
		...
		if (dEf.knockback) { // выполнить отбрасывание
			...
			// применить скорость отскока к компанонету Rigidbody
			knockbackVel = delta * knockbackSpeed;
			rigid.velocity = knockbackVel;

			// установить режим knockback и время прекращения отбрасывани
